...
def write_predictions(path_submit_data, preds):
    with open(path_submit_data) as f_data:
    ...
    with open(path_submit_data, 'wb') as f_data:
        writer = csv.DictWriter(f_data, delimiter='\t', fieldnames=reader.fieldnames)
        writer.writeheader()
        for datum in data:
...
def tune_passive_aggressive_T(best_L, *args):
    def train_fn(features, labels, T):
        return p1.average_passive_aggressive(features, labels, T, best_L)
    return tune(train_fn, *args)

def tune_passive_aggressive_L(best_T, *args):
    def train_fn(features, labels, L):
        return p1.average_passive_aggressive(features, labels, best_T, L)
    return tune(train_fn, *args)
