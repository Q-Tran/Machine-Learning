...
def hinge_loss(feature_matrix, labels, theta, theta_0):
    ...
    return max(0, 1-labels*(np.dot(theta,feature_matrix)) + theta_0)
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    counter = 0
    theta = 0 #change to zero vector with correct dimensions
    theta_0 = 0
    Tcounter=0
    i = feature_matrix.shape()[0]#numcolumns
    while(Tcounter<T):
        while(counter < i):
            if(Labels[i] * np.dot(np.transpose(current_theta),feature_matrix[:,i]) <= 0):
...
def passive_aggressive_single_step_update(feature_vector, label, L, current_theta, current_theta_0):
    ...
    completed.
    """
    learning_coefficient = min(hinge_loss(feasture_vector, Label, current_theta, theta_0)/np.dot(feature_vector, feature_vector), 1/L)
    next_theta = np.add(current_theta, learning_coefficient * np.dot(label, feature_vector))
    next_theta_0 = current_theta_0 + learning_coefficient * label
    tup1 = (next_theta, next_theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    while(Tcounter<T):
        ...
        while(counter < i):
            if(Labels[i] * np.dot(np.transpose(current_theta),feature_matrix[:,i]) <= 0):
                perceptron_single_step_update(feature_matrix[:,i], Labels[i], L, theta, theta_0)
                total_theta = theta + total_theta
                total_theta_0 = theta_0 + total_theta_0
            counter+=1
        Tcounter+=1
    average_theta = total_theta/(Tcounter*i + counter)
    average_theta_0 = total_theta_0/(Tcounter*i + counter)
    tup1 = (average_theta, average_theta_0)
...
def average_passive_aggressive(feature_matrix, labels, T, L):
    ...
    while(Tcounter<T):
        ...
        while(counter < i):
            if(Labels[i] * np.dot(np.transpose(current_theta),feature_matrix[:,i]) <= 0):
                passive_aggressive_single_step_update(feature_matrix[:,i], Labels[i], theta, theta_0)
                total_theta = theta + total_theta
                total_theta_0 = theta_0 + total_theta_0
            counter+=1
        Tcounter+=1
